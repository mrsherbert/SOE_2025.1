...
def binary_thresholder(img):
    ...
    adapt_white_hsv = cv2.adaptiveThreshold(roi[:,:,2], 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 161, -45)
...
def main():
    ...
    frame2 = cv2.imread("imagens_15_02_0137.png")
    ...
    try:
        while True:
            try:
                ...
                warped_frame, perspective_transform = warp(frame2)
                ...
                lane_overlay, avg_curvature = project_lane_info(
                    frame2[:, :, 0:3], img_bin, ploty, left_fitx, right_fitx,
